...
: fFileType(io::InputData::kFileTypeInvalid)
, fFileName("")
, fInfo("[none]")
{
  this->Reset(fileName);
...
(const std::string& fileName)
{
  ...
  const bf::path inputPath(fileName);
  if (bf::exists(inputPath))
  {
    if (inputPath.has_extension())
    {
      if (inputPath.extension().compare(std::string(".rmv")) == 0)
      {
        this->fFileType = io::InputData::kFileTypeRMV;
      }
      else if (inputPath.extension().compare(std::string(".nvm")) == 0)
      {
        this->fFileType = io::InputData::kFileTypeNVM;
      }
      else if (inputPath.extension().compare(std::string(".cmvs")) == 0)
      {
        this->fFileType = io::InputData::kFileTypeCMVS;
      }
      else if (inputPath.extension().compare(std::string(".ply")) == 0)
      {
        this->fFileType = io::InputData::kFileTypePLY;
      }
      else if (inputPath.extension().compare(std::string(".dense")) == 0)
      {
        this->fFileType = io::InputData::kFileTypeDENSE;
      }
      else
      {
        this->fFileType = io::InputData::kFileTypeInvalid;
      }
    }
  }
